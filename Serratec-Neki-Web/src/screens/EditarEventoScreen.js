...
import { atualizarEvento } from '../services/api';
...
export default function EditarEventoScreen({ route, navigation }) {
  ...
  useEffect(() => {
    if (evento.data) {
      const [datePart, timePart] = evento.data.split('T');
      const [anoEvento, mesEvento, diaEvento] = datePart.split('-');
      const [horaEvento, minutoEvento] = timePart.split(':');
      
      setAno(anoEvento);
      setMes(mesEvento);
      setDia(diaEvento);
      setHora(horaEvento);
      setMinuto(minutoEvento);
    }
  }, [evento.data]);
  ...
  const handleSalvar = async () => {
    if (!dia || !mes || !ano || !hora || !minuto || !localizacao) {
    ...
    try {
      ...
      Alert.alert('Erro', error.response?.data || 'Erro ao atualizar evento');
    } finally {
      setLoading(false);
    }
  };
    ...
    <ScrollView style={styles.container}>
      <View style={styles.box}>
        ...
        <TextInput
          ...
          editable={false}
        />
        <Text style={styles.hint}>O nome não pode ser editado</Text>
